...
def convimg(name):
    ...
    img = img.convert('RGB', matrix=None, dither=None, colors=4)
    pixels = img.getdata()
    colors = []
    hex_colors = []
    ...
    if len(colors) > 4:
        ...
        print ("Warning! Image has only " + str(
            len(colors)) + " unique colors (instead of exactly 4.) Color table will amended.")
    ...
    if len(colors) == 2:
        ...
        print "Warning: This image only seems to have a single, solid color. How do you expect " \
			  "that to work, cap'n?"
